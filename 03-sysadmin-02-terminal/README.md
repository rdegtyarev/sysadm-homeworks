...
7. Выполните команду `bash 5>&1`. К чему она приведет? Что будет, если вы выполните `echo netology > /proc/$$/fd/5`? Почему так происходит?  
   **Решение:**  
   `bash 5>&1` - создает файловый дескриптор `5` в поток вывода (stdout) текущего `bash`  
   `echo netology > /proc/$$/fd/5` - передать `netology` в `stdout` созданного дескриптора. Результатом будет вывод `netology` в `stdoud` текущего `bash`.  
